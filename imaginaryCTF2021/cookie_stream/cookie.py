...
def backend():
    if request.method == 'POST':
        ...
        nonce = urandom(8)
        # my friend told me that cbc had some weird bit flipping attack? ctr sounds way cooler anyways
        cipher = AES.new(key, AES.MODE_CTR, nonce=nonce)
        cookie = hexlify(
            nonce + cipher.encrypt(pad(request.form['username'].encode(), 16)))
...
def home():
    ...
    cipher = AES.new(key, AES.MODE_CTR, nonce=nonce)
    username = unpad(cipher.decrypt(
        unhexlify(request.cookies.get('auth')[16:])), 16).decode()
    if username == 'admin':
